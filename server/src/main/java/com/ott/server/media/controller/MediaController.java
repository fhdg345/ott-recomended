...
import com.ott.server.genre.service.GenreService;
...
public class MediaController {
    ...
    public MediaController(MediaService mediaService, MediaRepository mediaRepository, GenreService genreService, GenreRepository genreRepository, MediaOttRepository ottRepository) {
    ...
    public ResponseEntity<MultiResponseDto> getMediasByTv(
        ...
        MultiResponseDto medias = mediaService.getMedias("TV", genres,  pageable);
    ...
    public ResponseEntity<MultiResponseDto> getMediasByMovie(
        ...
        // 중복 제거 후 리스트로 변환
        List<String> genreList = new ArrayList<>(new HashSet<>(genre)); // 장르 리스트
        List<String> ottList = new ArrayList<>(new HashSet<>(ott)); // OTT 리스트


        Pageable pageable = PageRequest.of(page - 1, size);
        //List<Genre> genres = genreRepository.findByGenreNameIn(genreList);

        List<Genre> top36Genres = genreService.findTop36Genres(genreList);
        log.info("****************** top12Genres ======================>  {}" ,  top36Genres);
//        List<MediaOtt> otts = ottRepository.findByOttNameIn(ottList);

        MultiResponseDto medias = mediaService.getMedias("영화", top36Genres,  pageable);
