...
export const GetMember = (): Promise<NewMember> =>
  instance.get('/members').then((res) => {
    let avatarUri = null;
    if( res.data.avatarUri.includes("https://") || res.data.avatarUri.includes("http://") ){
      avatarUri = res.data.avatarUri;
    } else {
      avatarUri = import.meta.env.VITE_AVATAR_URL + res.data.avatarUri;
    }
    res.data.avatarUri = avatarUri;
    return res.data;
  });
